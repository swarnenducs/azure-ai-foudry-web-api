  ...
  app_settings = merge(
    {
      ...
      LLM_API_VERSION                = var.routing_llm_api_version
      LOG_LEVEL                      = var.log_level
      LOG_JSON                       = var.log_json ? "true" : "false"
    },
    var.fabric_routing_mode != null ? { FABRIC_ROUTING_MODE = var.fabric_routing_mode } : {},
    var.routing_llm_endpoint != null ? { LLM_ENDPOINT = var.routing_llm_endpoint } : {},
    var.routing_llm_deployment != null ? { LLM_DEPLOYMENT = var.routing_llm_deployment } : {},
